...
- `--showquerygff` / `--showtargetgff` are **not upstream-byte-compatible**. Rust currently emits project-specific GFF3 records, while upstream emits wrapped GFF2 dumps with model-specific similarity/gene/exon/intron attributes. This work is explicitly deferred.
