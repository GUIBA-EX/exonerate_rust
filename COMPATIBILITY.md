...
| `--forwardcoordinates yes|no` | implemented; default forward-reference coordinates and reverse-oriented compatibility mode verified |
...
| `genome2genome` UTR and CDS query/target/joint introns, CDS phase 0/1/2 split codons, codon gaps, bilateral frameshifts, and four query/target strand combinations | implemented; upstream 557-point joint-intron oracle, all-strand report oracle, plus phase-specific query/joint trace and coordinate-conservation regressions verified |
...
| common `--ryo` identifiers (including full FASTA `%qd`/`%td` definitions), aligned/coding ranges, equivalenced `%e...` and `%p...` statistics, score, rank, model, sugar/cigar/vulgar blocks, escapes, and scored raw `%P...` transitions | implemented for current models; composed cDNA/genome and genome/genome `%P` transition oracles are exact |
| `--subopt yes` equivalenced-pair exclusion | implemented for affine local and current non-affine models; codon MATCH transitions block all three nucleotide pairs, with `coding2coding` and `coding2genome` order-oracled; full remaining tie-order audit remains open |
...
| checkpoint traceback and `-D`/`--dpmemory` dispatch | implemented for DNA/protein affine scopes, `est2genome`, phase-intron/frameshift `protein2genome`, `coding2genome`, composed `cdna2genome`, and `genome2genome`; g2g currently replays prefix rows per parent block, while exact MiB layout remains open |
